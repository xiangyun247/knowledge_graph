...
# 多 LLM 模型切换：当前请求的 (api_key, base_url, model)，由 run_agent/run_agent_stream 设置
_llm_config_ctx: contextvars.ContextVar[Optional[tuple]] = contextvars.ContextVar("llm_model_config", default=None)


# 6.3 系统提示：角色、边界、引用要求（M18：面向老年/照护者时简短句、分条、通俗解释）
AGENT_SYSTEM_PROMPT = """你是一名医学知识图谱与文献检索助手，负责基于「知识图谱」和「文档知识库」回答用户问题。本系统面向老龄认知障碍人群及照护者，回答时请兼顾低认知负荷。
...
7. 【适老与照护者】当回答面向老年用户或照护者时：优先使用简短句、分条列举（如「第一…第二…」或自然分段），避免长段落；对医学术语请给出通俗解释或在括号内简要说明；关键信息可适当重复或放在句首，便于理解与记忆。
8. 【患者教育】当用户明确要求「患者教育」「写一篇给患者看的说明」「通俗解释给患者」「出院注意事项」等时，应调用 generate_patient_education 工具，将用户的问题或主题作为 topic 参数传入。该工具会基于图谱和文献自动生成通俗易懂的患者教育短文，你只需将工具返回的内容呈现给用户即可。
9. 【科普推文】当用户明确要求「科普推文」「写一条微博」「发朋友圈文案」「推文版」等时，应调用 generate_science_tweet 工具。参数：topic 为主题；word_limit 默认 140；style 如「轻松」「严谨」；若有已有回答内容可传入 source_content。输出 1～3 条推文 + 话题标签建议。"""
...
def _get_llm(base_url: str = None, model: str = None, api_key: str = None):
    """优先使用传入参数，其次使用 _llm_config_ctx（多模型切换），最后使用 DEEPSEEK 默认。"""
    cfg = _llm_config_ctx.get()
    if cfg:
        ck, cb, cm = cfg
...
def _build_initial_messages(
    ...
    if (answer_style or "").strip().lower() == "concise":
        user_content = "【用户希望简洁回答】请用三步以内简要回答，每步一句话。\n\n" + question
...
def run_agent(
    ...
    *,
    ...
    - model_id: 多 LLM 支持，如 deepseek-chat、qwen-plus 等，需在 .env 配置对应 API Key。
...
def run_agent_stream(
    ...
    *,
    ...
    - model_id: 多 LLM 支持，如 deepseek-chat、qwen-plus 等。

...
async def get_graph_data(params: Optional[Dict[str, Any]] = None):
    ...
    try:
        ...
        for gi, graph in enumerate(graphs):
            ...
            # 仅在第一张图上采样几个节点到日志，便于分析字段结构
            if gi == 0:
                try:
                    import json as _json
                    for idx, sample_node in enumerate(nodes[:3]):
                        logger.info(
                            "[graph_data] sample node %s: %s",
                            idx,
                            _json.dumps(sample_node, ensure_ascii=False)[:500],
                        )
                except Exception:
                    logger.info("[graph_data] sample nodes (repr): %r", nodes[:3])
